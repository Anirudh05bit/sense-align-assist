...
export class AudioService {
  ...
  /**
   * Play a short audio cue (beep) for status feedback.
   * Useful for visually impaired users to know when assistant is listening or processing.
   * 
   * @param frequency Frequency in Hz
   * @param duration Duration in seconds
   * @param type Waveform type
   */
  public async playAudioCue(frequency: number = 440, duration: number = 0.1, type: OscillatorType = 'sine'): Promise<void> {
    try {
      await this.initAudioContext();
      if (!this.audioContext) return;
      ...
      console.warn('[AudioService] Failed to play audio cue', err);
    }
  }
